...
def safe_log_config(config: Dict) -> Dict:
    ...
    sensitive_keys = ['api_key', 'API_KEY', 'openai_api_key',
                      'anthropic_api_key', 'google_api_key', 'secret_key', 'password']
...
def safe_error_traceback(traceback_str: str) -> str:
    ...
    for pattern in patterns:
        safe_traceback = re.sub(
            pattern, '***HIDDEN_API_KEY***', safe_traceback)
...
class WebMessageBuffer:
    def __init__(self, session_id):
        ...
        self.agent_status = {
            ...
            "Factor Researcher": "pending",
            "Strategy Designer": "pending",
            "Backtest Runner": "pending",
            "Strategy Evaluator": "pending",
        }
        ...
        for collection in collections:
            if collection.name.endswith(f"_{session_id}"):
                try:
                    ...
                    print(
                        f"[WARNING] Failed to cleanup collection {collection.name}: {e}")
    except Exception as e:
        print(
            f"[WARNING] Failed to cleanup collections for session {session_id}: {e}")
...
def run_analysis_background(session_id: str, config: Dict):
    ...
    try:
        ...
        buffer.add_message(
            "System", f"Initializing analysis for {config['ticker']}...")
        ...
        if not is_production():
            ...
            print(
                f"[DEBUG] Creating initial state for {config['ticker']} on {config['analysis_date']}")
        ...
        buffer.add_message(
            "System", f"Starting analysis for {config['ticker']} on {config['analysis_date']}")
            ...
            if len(chunk.get("messages", [])) > 0:
                ...
                if "market_report" in chunk and chunk["market_report"]:
                    ...
                    buffer.update_progress(
                        progress, "Market analysis completed")
                ...
                if "sentiment_report" in chunk and chunk["sentiment_report"]:
                    ...
                    buffer.update_progress(
                        progress, "Social sentiment analysis completed")
                ...
                if "fundamentals_report" in chunk and chunk["fundamentals_report"]:
                    ...
                    buffer.update_progress(
                        progress, "Fundamentals analysis completed")
                ...
                if "investment_debate_state" in chunk and chunk["investment_debate_state"]:
                    ...
                    if "bull_history" in debate_state and debate_state["bull_history"]:
                        ...
                        if latest_bull and len(latest_bull.strip()) > 0:
                            buffer.add_message(
                                "Bull Researcher", f"Bull Analysis: {latest_bull}")
                    ...
                    if "bear_history" in debate_state and debate_state["bear_history"]:
                        ...
                        if latest_bear and len(latest_bear.strip()) > 0:
                            buffer.add_message(
                                "Bear Researcher", f"Bear Analysis: {latest_bear}")
                    ...
                    if "judge_decision" in debate_state and debate_state["judge_decision"]:
                        ...
                        buffer.add_message(
                            "Research Manager", f"Final Decision: {debate_state['judge_decision']}")
                        buffer.update_progress(
                            progress, "Research team decision completed")

                # Handle quantitative research and strategy updates
                if ("factors_spec" in chunk and chunk["factors_spec"]) or (
                    "factor_values" in chunk and chunk["factor_values"]
                ):
                    factors_spec = chunk.get("factors_spec", "")
                    factor_values = chunk.get("factor_values", "")
                    factor_section = "## Factor Specification\n\n"
                    if factors_spec:
                        factor_section += f"{factors_spec}\n\n"
                    factor_section += "## Factor Values (JSON)\n\n"
                    if isinstance(factor_values, str):
                        factor_section += f"```json\n{factor_values}\n```"
                    else:
                        factor_section += "```json\n" + \
                            json.dumps(factor_values, indent=2) + "\n```"

                    buffer.update_report_section(
                        "factor_research", factor_section)
                    buffer.update_agent_status(
                        "Factor Researcher", "completed")
                    buffer.update_progress(
                        progress, "Quantitative factor research completed")

                if "strategy_template" in chunk and chunk["strategy_template"]:
                    buffer.update_report_section(
                        "strategy_design", chunk["strategy_template"])
                    buffer.update_agent_status(
                        "Strategy Designer", "completed")
                    buffer.update_progress(
                        progress, "Strategy design completed")

                if "backtest_results" in chunk and chunk["backtest_results"]:
                    backtest_results = chunk["backtest_results"]
                    if isinstance(backtest_results, (dict, list)):
                        backtest_text = json.dumps(backtest_results, indent=2)
                    else:
                        backtest_text = str(backtest_results)

                    summary = chunk.get("selected_strategy_summary", "")
                    backtest_section = ""
                    if summary:
                        backtest_section += f"{summary}\n\n"
                    backtest_section += "```json\n" + backtest_text + "\n```"

                    buffer.update_report_section(
                        "backtest_results", backtest_section)
                    buffer.update_agent_status("Backtest Runner", "completed")
                    buffer.update_progress(progress, "Backtest completed")

                if "strategy_acceptable" in chunk:
                    acceptable = chunk["strategy_acceptable"]
                    evaluation_text = (
                        "Strategy evaluation result: ACCEPTABLE."
                        if acceptable
                        else "Strategy evaluation result: NOT ACCEPTABLE."
                    )
                    buffer.update_report_section(
                        "strategy_evaluation", evaluation_text)
                    buffer.update_agent_status(
                        "Strategy Evaluator", "completed")
                    buffer.update_progress(
                        progress, "Strategy evaluation completed")
        ...
        buffer.add_message(
            "Error", f"Detailed error: {safe_error_traceback(error_traceback)}")
